...
def sub_bytes(s, sbox=s_box):
    n = len(s)
    sbox_ = array2matrix(sbox, 16)
    return [[sbox_[int(hex(s[i][j])[2:].zfill(2)[0], 16)][int(hex(s[i][j])[2:].zfill(2)[1], 16)] for j in range(n)] for i in range(n)]
...
def decrypt(key, ciphertext):
    ...
    inv_shift_rows(ciphertext)
    plaintext = sub_bytes(ciphertext, sbox=inv_s_box)
